...
import {
    ...
    Send, ShieldAlert, Sparkles, Smile, Image as ImageIcon, HeartOff, Menu, MoreVertical, Crown, ToggleRight, User, Lock
} from "lucide-react";
...
export default function SmartMatchPage() {
    ...
    const [myTier, setMyTier] = useState("FREE"); // Track My Subscription
    ...
    const [partner, setPartner] = useState<any>(null); // Contains basic info
    const [partnerFull, setPartnerFull] = useState<any>(null); // NEW: Full info for modal
    ...
    // --- NEW: FETCH FULL PARTNER PROFILE FOR MODAL ---
    const openPartnerProfile = async () => {
        if (!partner?.id) return; // Need partner ID
        // We need to fetch the partner ID from the conversation participants first if not stored in 'partner' state
        // For now, assuming 'partner' state has the profile data. 
        // In loadChat, we set 'partner' to the profile object. We need the ID.

        // FETCH FULL DETAILS
        // (In a real app, you might need to query by ID if 'partner' is incomplete)
    ...
    const loadChat = async (convId: string, currentUserId: string) => {
        ...
        if (conv) {
            ...
            if (other) setPartner(other.profiles); // Stores full profile in partner state
        }
                    ...
                    <div className="pt-12 px-6 pb-6 space-y-6">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                                {partnerFull.full_name || "Anonymous"}
                                {partnerFull.tier === 'PRO' && <Crown size={20} className="text-yellow-400 fill-yellow-400" />}
                            </h2>

                            {/* AGE SECTION - ALWAYS BLURRED FOR OTHERS */}
                            <div className="inline-flex items-center gap-2 mt-1 px-3 py-1 bg-zinc-800 rounded-full border border-zinc-700">
                                ...
                                <div className="h-4 w-8 bg-zinc-600 rounded blur-sm opacity-50 relative overflow-hidden">
                                    {/* Fake Blur Visual */}
                                </div>
                        ...
                        <div className="space-y-2">
                            ...
                            {isFree && (
                                <div className="absolute inset-x-0 top-1/2 flex justify-center pointer-events-none">
                                    <span className="bg-black/80 px-3 py-1 rounded-full text-[10px] font-bold text-[#FF6B91] border border-[#FF6B91]/30">UPGRADE TO VIEW</span>
                                </div>
                            )}
                        </div>

                        {/* INTERESTS - BLURRED IF FREE */}
                        <div className="space-y-2">
                            <h3 className="text-xs font-bold text-zinc-500 uppercase">Interests</h3>
                            <div className={`flex flex-wrap gap-2 ${isFree ? 'blur-sm select-none opacity-50' : ''}`}>
                                {(partnerFull.interests || ["Music", "Art", "Tech"]).map((tag: string, i: number) => (
                                    <span key={i} className="px-3 py-1 bg-zinc-800 rounded-full text-xs text-white border border-zinc-700">{tag}</span>
                                ))}
                            </div>
                        </div>

                        <button onClick={() => router.push('/pricing')} className="w-full py-3 bg-gradient-to-r from-[#FF6B91] to-[#A67CFF] rounded-xl font-bold text-white text-sm shadow-lg">
                            {isFree ? "Unlock Profile Details" : "View Full Profile"}
                        </button>
                    </div>
                </div>
            </div>
